...
public class CSV { 
	 ...
	 * @throws NickDuplicadoException 
	 */
	public static void processCSV( File file ) 
	throws IOException // Error de I/O
, NickDuplicadoException, IdDuplicadoException
	{
...
, NickDuplicadoException, IdDuplicadoException
	...
	private static HashMap <String, UsuarioTwitter> mapaId = new HashMap<>();
	
	private static int numLin = 0;
	private static void procesaLineaDatos( ArrayList<Object> datos ) throws NickDuplicadoException, IdDuplicadoException {
		// TODO Cambiar este proceso si se quiere hacer algo con las cabeceras
        ...
        if (mapaId.containsKey(id)) {
        	throw new IdDuplicadoException("Error: Id duplicado encontrado - " + id);
        }
        
        UsuarioTwitter usu = new UsuarioTwitter(id, screenName, tags, avatar, followersCount, friendsCount, lang, lastSeen, tweetId, friends);
        System.out.println(usu);
     	mapaId.put(id, usu);
		numLin++;
		mapaClaveUsuario(datos);
		//System.out.println( numLin + "\t" + datos );  // Saca la cabecera por consola de error
		//System.out.println(mapa);
	}
	
	private static HashMap <String, UsuarioTwitter> mapaNick = new HashMap<>();
	
	public static void mapaClaveUsuario(ArrayList<Object> datos) throws NickDuplicadoException{
        ...
        UsuarioTwitter usu = new UsuarioTwitter(id, screenName, tags, avatar, followersCount, friendsCount, lang, lastSeen, tweetId, friends);
        if (mapaNick.containsKey(avatar)) {
        	throw new NickDuplicadoException("Error: Nick duplicado encontrado - " + avatar);
        }
        mapaNick.put(avatar, usu);
	}

	 public static class NickDuplicadoException extends Exception {
	        public NickDuplicadoException(String mensaje) {
	            super(mensaje);
	        }
	    }
	 
	 public static class IdDuplicadoException extends Exception {
	        public IdDuplicadoException(String mensaje) {
	            super(mensaje);
	        }
	    }
	
	 public static void comprobarListaAmigos() {
		    for (Entry<String, UsuarioTwitter> entry : mapaNick.entrySet()) {
		        String usuario = entry.getKey();
		        ArrayList<String> listaAmigos = ((UsuarioTwitter) entry).getFriends();

		        for (String amigoId : listaAmigos) {
		            if (mapaId.containsKey(amigoId)) {
		                System.out.println("Usuario con amigos en el sistema: " + usuario);
		                arbolAmigos.add(usuario);
		                break; // No necesitamos seguir verificando si ya encontramos al menos un amigo en el sistema
		            }
		        }
		    }
		}
	 private static TreeSet<String> arbolAmigos = new TreeSet<>();
